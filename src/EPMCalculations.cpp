      ...
      if(type == 2)
        {
          ...
          f->read(&readv[0], readv.size());
          std::memcpy(&val, &readv[0], sizeof(val));
          mpf_class INIT = mpf_class(2451545.0) + mpf_class(val) / mpf_class(86400);
          ...
          f->read(&readv[0], readv.size());
          std::memcpy(&val, &readv[0], sizeof(val));
          mpf_class MID = mpf_class(2451545.0) + mpf_class(val) / mpf_class(86400);
          readv.clear();
          readv.resize(8);
          f->read(&readv[0], readv.size());
          std::memcpy(&val, &readv[0], sizeof(val));
          ...
          if(var == 0)
            {
              ...
              for(size_t i = 1; i <= n / 2; i++)
                {
                  flag = -flag;
                  W[2 * i] = 0.5 * (P[2 * i + 1] / mpf_class(static_cast<double>(2 * i + 1))
                                    - P[2 * i - 1] / mpf_class(static_cast<double>(2 * i - 1)));
                  if(2 * i + 2 < P.size())
                    {
                      W[2 * i + 1] = 0.5 * (P[2 * i + 2] / mpf_class(static_cast<double>(2 * i + 2))
                                            - P[2 * i] / mpf_class(static_cast<double>(2 * i)))
                                     + flag * 0.25 * (1.0 / mpf_class(static_cast<double>(i)) + 1.0 / mpf_class(
