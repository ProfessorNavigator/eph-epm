      ...
      for(double i = JD; i < JD + step * stepnum; i = i + step)
        {
          ...
          if(body != "moon" && body != "earth")
            {
              X = epm.bodyCalcX(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au);
              Y = epm.bodyCalcY(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au);
              Z = epm.bodyCalcZ(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au);
            }
          else
            {
              if(body == "earth")
                {
                  X = epm.bodyCalcX(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au)
                      - epm.bodyCalcX(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au) / rho;
                  Y = epm.bodyCalcY(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au)
                      - epm.bodyCalcY(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au) / rho;
                  Z = epm.bodyCalcZ(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au)
                      - epm.bodyCalcZ(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au) / rho;
                }
              else
                {
                  mpf_class moonx, moony, moonz;
                  moonx = epm.bodyCalcX(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au);
                  moony = epm.bodyCalcY(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au);
                  moonz = epm.bodyCalcZ(&ephfile, &moonb, &moone, JDfin, xyz, bodymoontype, &au);
                  X = epm.bodyCalcX(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au)
                      - moonx / rho + moonx;
                  Y = epm.bodyCalcY(&ephfile, &bodyb, &bodye, JDfin, xyz, bodytype, &au)
                      - moony / rho + moony;
